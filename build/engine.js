...
class Engine extends basic_kodyfire_1.Engine {
    ...
    gifAddFrame(page, encoder, width, height) {
        return __awaiter(this, void 0, void 0, function* () {
            // Get root of page
            ...
            // Take screenshot
            // @ts-ignore
            const pngBuffer = yield rootHandle.screenshot({
                clip: { width, height, x: 0, y: 0 },
                omitBackground: true
            });
            const png = new PNG(pngBuffer);
            yield this.decode(png).then(pixels => encoder.addFrame(pixels));
        });
    }
    ...
    overwrite = false) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (debug) {
                yield fsPromises.writeFile(filename.replaceAll('.gif', '.html'), content);
            }
            // record gif
            ...
            for (let i = 0; i < 10; i++) {
                yield this.gifAddFrame(page, encoder, width, height);
            }
